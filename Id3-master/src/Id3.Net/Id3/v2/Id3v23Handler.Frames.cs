...
namespace Id3.v2
{
    internal sealed partial class Id3V23Handler
    {
        ...
        private static byte[] EncodeLyrics(Id3Frame id3Frame)
        {
            ...
            bytes.AddRange(!string.IsNullOrEmpty(frame.Language.ToString())
                ? defaultEncoding.GetBytes(frame.Language.ToString())
                : defaultEncoding.GetBytes("eng"));
            ...
            Encoding lyricsEncoding = TextEncodingHelper.GetEncoding(frame.EncodingType);
            bytes.AddRange(lyricsEncoding.GetPreamble());
            if (!string.IsNullOrEmpty(frame.Lyrics))
                bytes.AddRange(descriptionEncoding.GetBytes(frame.Lyrics));
            bytes.AddRange(TextEncodingHelper.GetSplitterBytes(frame.EncodingType));
